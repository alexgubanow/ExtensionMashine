...
namespace UsbHIDControl.ViewModels
{
    public class UsbHIDViewModel : BindableBase, IDisposable
    {
        ...
        public UsbHIDViewModel(IEventAggregator ea)
        {
            ...
            ea.GetEvent<GetHX711RawDataEvent>().Subscribe(() => GetHX711RawData());
        }

        void GetHX711RawData()
        {
            try
            {
                if (IsConnected)
                {
                    ...
                    data[0] = (byte)usbReports_t.getHX711val;
                    Hidapiw.GetFeatureReport(devIdx, ref data);
                    while (data[1] == 0xFF && data[2] == 0xFF && data[3] == 0xFF && data[4] == 0xFF)
                    {
                        Hidapiw.GetFeatureReport(devIdx, ref data);
                    }
                    _eventAggregator.GetEvent<NewHX711valueEvent>().Publish(data[1] << 24 | data[2] << 16 | data[3] << 8 | data[4]);
                }
            }
            catch (SEHException e)
            {
